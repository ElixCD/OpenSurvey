...
CREATE TABLE IF NOT EXISTS `Sysurvey`.`user_relations` (
  `iduser_relations` INT NOT NULL,
  `users_iduser_pollster` INT NOT NULL,
  `users_iduser_polled` INT NOT NULL,
  PRIMARY KEY (`iduser_relations`),
  INDEX `fk_user_relations_users1_idx` (`users_iduser_pollster` ASC),
  INDEX `fk_user_relations_users2_idx` (`users_iduser_polled` ASC),
  CONSTRAINT `fk_user_relations_users_pollster`
    FOREIGN KEY (`users_iduser_pollster`)
    REFERENCES `Sysurvey`.`users` (`iduser`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_user_relations_users_polled`
    FOREIGN KEY (`users_iduser_polled`)
    REFERENCES `Sysurvey`.`users` (`iduser`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
